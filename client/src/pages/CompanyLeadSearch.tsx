      ...
      <div className="flex min-h-[calc(100vh-4rem)]">
        ...
        <main className="flex-1 min-w-0 p-3 lg:p-5 space-y-3">
          ...
          <section className="rounded-xl border border-slate-800 bg-[#071321]/90 overflow-hidden">
            <div className="p-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><SectionTitle number="4" title="Results" inline /><span className="text-xs text-violet-300">› {searched && resultCount > 0 ? `${resultCount.toLocaleString()} businesses found (showing ${Math.min(visibleLeads.length, resultCount)} preview rows)` : "Set a target count to search"}</span></div><div className="flex flex-wrap items-center gap-2"><button onClick={handleExport} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-700 text-[11px] text-slate-300"><Download className="w-3.5 h-3.5" />Export <ChevronDown className="w-3 h-3" /></button><button className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-700 text-[11px] text-slate-300"><Database className="w-3.5 h-3.5" />Columns</button></div></div>{actionNotice && <div className="mx-4 mb-3 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-[10px] text-emerald-300">{actionNotice}</div>}<div className="mx-4 mb-3 flex flex-wrap items-center gap-2"><div className="inline-flex flex-wrap items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/40 p-2"><span className="text-[10px] uppercase tracking-wide text-slate-400 mr-1">Saved filters</span><input aria-label="Saved filter name" value={savedFilterName} onChange={(event) => setSavedFilterName(event.target.value)} placeholder="Name this filter" className="w-40 rounded-md border border-slate-700 bg-slate-900/70 px-2.5 py-1.5 text-[10px] text-white outline-none placeholder:text-slate-600" /><button onClick={handleSaveFilter} disabled={savedFiltersLoading} className="rounded-md bg-violet-600 px-3 py-1.5 text-[10px] font-semibold text-white hover:bg-violet-500 disabled:opacity-50">{savedFiltersLoading ? "Saving…" : "Save Filter"}</button>{savedFilterNotice && <span className="text-[10px] text-emerald-300">{savedFilterNotice}</span>}</div>{savedFilters.length > 0 && <div className="flex flex-wrap gap-2">{savedFilters.map((view) => <div key={view.id} className="inline-flex items-center rounded-md border border-violet-500/30 bg-violet-500/10"><button onClick={() => void handleApplySavedFilter(view)} className="px-2.5 py-1.5 text-[10px] text-violet-200 hover:text-white">{view.name}</button><button aria-label={`Delete saved filter ${view.name}`} onClick={() => void handleDeleteSavedFilter(view.id)} className="border-l border-violet-500/30 px-1.5 text-violet-300 hover:text-white"><X className="h-3 w-3" /></button></div>)}</div>}</div><div className="px-4 py-3 border-b border-slate-800 flex flex-wrap gap-2">{["All", "No Website", "Weak Website", "Outdated Website", "Poor Mobile", "Weak SEO", "Low Visibility", "Weak Reviews", "Weak Social Media"].map((tab) => { const value = tab === "Poor Mobile" ? "Poor Mobile Exp." : tab; const active = selectedTab === value; return <button key={tab} onClick={() => setSelectedTab(value)} className={`rounded-md border px-2.5 py-1.5 text-[10px] font-medium transition-colors ${active ? "border-violet-400 bg-white text-slate-950 ring-2 ring-violet-400/40" : "border-slate-700 bg-white text-slate-950 hover:bg-slate-100"}`}>{tab} {tab === "All" && searched ? `(${resultCount.toLocaleString()})` : ""}</button>; })}</div><div className="px-4 py-3 border-b border-slate-800 bg-[#06101c]"><div className="mb-2 text-[10px] uppercase tracking-wide text-slate-500">Contact &amp; business data</div><div className="flex flex-wrap gap-2">{availabilityFilterOptions.map((filter) => <button key={filter} aria-label={`Filter results by ${filter}`} title={`Filter results by ${filter}`} onClick={() => toggleDataFilter(filter)} className={`rounded-md border px-2.5 py-1.5 text-[10px] font-medium leading-tight ${selectedDataFilters.includes(filter) ? "border-blue-300 bg-blue-600 text-white ring-2 ring-blue-300/40" : "border-slate-300 bg-white text-slate-950 hover:bg-slate-100"}`}>{filter}{selectedDataFilters.includes(filter) ? " ✓" : ""}</button>)}</div></div>
            <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left"><thead className="bg-[#06101c] text-[10px] text-slate-500"><tr><th className="px-4 py-3"><input type="checkbox" aria-label="Select all" checked={visibleLeads.length > 0 && visibleLeads.every((lead) => selectedRowIds.includes(lead.id))} onChange={() => setSelectedRowIds(visibleLeads.every((lead) => selectedRowIds.includes(lead.id)) ? [] : visibleLeads.map((lead) => lead.id))} /></th><th className="px-2 py-3">Business Name</th><th className="px-2 py-3">Category</th><th className="px-2 py-3">Opportunity</th><th className="px-2 py-3">Score</th><th className="px-2 py-3"><button onClick={() => toggleDataFilter("Website")} className={`rounded-md border px-2 py-1 ${selectedDataFilters.includes("Website") ? "border-blue-300 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-950"}`}>Website {selectedDataFilters.includes("Website") ? "✓" : "⌄"}</button></th><th className="px-2 py-3"><button onClick={() => toggleDataFilter("Business Email")} className={`rounded-md border px-2 py-1 ${selectedDataFilters.includes("Business Email") ? "border-blue-300 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-950"}`}>Business Email {selectedDataFilters.includes("Business Email") ? "✓" : "⌄"}</button></th><th className="px-2 py-3"><button onClick={() => toggleDataFilter("Phone Number")} className={`rounded-md border px-2 py-1 ${selectedDataFilters.includes("Phone Number") ? "border-blue-300 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-950"}`}>Phone {selectedDataFilters.includes("Phone Number") ? "✓" : "⌄"}</button></th><th className="px-2 py-3"><button onClick={() => toggleDataFilter("WhatsApp Number")} className={`rounded-md border px-2 py-1 ${selectedDataFilters.includes("WhatsApp Number") ? "border-blue-300 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-950"}`}>WhatsApp {selectedDataFilters.includes("WhatsApp Number") ? "✓" : "⌄"}</button></th><th className="px-2 py-3"><button onClick={() => toggleDataFilter("Google Business Profile")} className={`rounded-md border px-2 py-1 ${selectedDataFilters.includes("Google Business Profile") ? "border-blue-300 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-950"}`}>Google Profile {selectedDataFilters.includes("Google Business Profile") ? "✓" : "⌄"}</button></th><th className="px-2 py-3">Actions</th></tr></thead><tbody className="divide-y divide-slate-800/80">{visibleLeads.map((lead) => <tr key={lead.id} onClick={() => setSelectedLeadId(lead.id)} className={`text-[10px] cursor-pointer ${selectedLeadId === lead.id ? "bg-violet-500/8" : "hover:bg-slate-900/70"}`}><td className="px-4 py-3"><input type="checkbox" aria-label={`Select ${lead.company}`} checked={selectedRowIds.includes(lead.id)} onClick={(event) => event.stopPropagation()} onChange={() => toggleRowSelection(lead.id)} /></td><td className="px-2 py-3"><div className="font-medium text-white">{lead.company}</div><div className="text-[9px] text-slate-500">{selectedCity ? `${selectedCity}, ` : ""}{selectedCountry || lead.location}</div></td><td className="px-2 py-3 text-slate-400">{lead.category}</td><td className="px-2 py-3"><span className="px-2 py-1 rounded bg-violet-500/20 text-violet-200">{lead.opportunity}</span></td><td className="px-2 py-3"><span className={`inline-grid place-items-center w-8 h-8 rounded-full border ${scoreClass(lead.score)}`}>{lead.score}</span></td><td className="px-2 py-3 text-slate-400"><span className="truncate" title={getLeadDataValue(lead, "Website")}>{getLeadDataValue(lead, "Website") || "—"}</span></td><td className="px-2 py-3 text-slate-400 max-w-[150px] truncate"><span title={getLeadDataValue(lead, "Business Email")}>{getLeadDataValue(lead, "Business Email") || "—"}</span></td><td className="px-2 py-3 text-slate-400">{getLeadDataValue(lead, "Phone Number") || "—"}</td><td className="px-2 py-3 text-emerald-400">{getLeadDataValue(lead, "WhatsApp Number") || "—"}</td><td className="px-2 py-3 text-red-400">{getLeadDataValue(lead, "Google Business Profile") ? "Available" : "—"}</td><td className="px-2 py-3"><div className="flex items-center gap-1"><button className="p-1.5 rounded hover:bg-slate-800"><Eye className="w-3.5 h-3.5 text-slate-400" /></button><button className="p-1.5 rounded hover:bg-slate-800"><MoreHorizontal className="w-3.5 h-3.5 text-slate-400" /></button></div></td></tr>)}</tbody></table></div>
          </section>
