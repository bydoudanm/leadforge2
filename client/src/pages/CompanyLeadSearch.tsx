...
const leads: Lead[] = [
  ...
  {
    id: 7,
    company: "Atlas Commerce Austin Demo",
    location: "Austin, TX",
    category: "E-commerce",
    opportunity: "Weak Website",
    score: 87,
    ...
    parentCompanyId: "atlas-commerce-demo",
    email: "austin@atlascommerce.demo",
    founderEmail: "founder@atlascommerce.demo",
    phone: "(512) 555-0184",
    facebookPage: "facebook.com/atlascommerceaustin",
    instagram: "instagram.com/atlascommerceaustin",
    linkedinProfile: "linkedin.com/company/atlas-commerce-austin-demo",
    googleProfile: "https://maps.google.com/?q=Atlas+Commerce+Austin+Demo",
    website: "austin.atlascommerce.demo",
    reason: "Demo company fixture with parent-level social and news signals",
    service: "Multi-location Website Growth",
    reviews: "No review data supplied for this demo fixture",
...
export default function CompanyLeadSearch() {
  ...
  const companyLeads = useMemo(() => leads.map((lead) => {
    ...
    return {
      ...
      parentFacebookPage: hierarchy.parentFacebookPage,
      parentInstagram: hierarchy.parentInstagram,
      parentXProfile: hierarchy.parentXProfile,
    };
  }), []);
  const filteredLeads = useMemo(() => {
    ...
    const businessTypeMatches = filterBusinessTypeResults(companyLeads, query, businessTypeMatchMode, (lead) => `${lead.category} ${lead.company}`);
    const companyMatches = businessTypeMatches.filter((lead) => matchesCompanyProfileFilters(lead, employeeRange, revenueRange, selectedEntityType));
  ...
  }, [businessType, businessTypeMatchMode, companyLeads, selectedAnnualRevenue, selectedDataFilters, selectedEmployeeSize, selectedTab, selectedEntityType]);
  ...
  const resultCount = searched && selectedTab === "All" && selectedDataFilters.length === 0 && !hasCompanyFilters && !hasBusinessTypeFilter
    ? requestedCount
    ...
    try {
      const response = await fetch("/api/ai/generate-outreach", {
        ...
        body: JSON.stringify({
          ...
          socialProfiles: [selectedCompany.parentLinkedinProfile, selectedCompany.parentFacebookPage, selectedCompany.parentInstagram, selectedCompany.parentXProfile].filter(Boolean),
          latestNews: selectedCompany.latestNews?.headline,
        }),
      });
  ...
  return (
    <div data-search-mode={searchMode} className="min-h-screen bg-[#020914] text-slate-200 overflow-x-hidden">
      ...
      <div className="flex min-h-[calc(100vh-4rem)]">
        <AcquisitionSidebar activeMode="company" expanded={sidebarOpen} onToggle={() => setSidebarOpen((open) => !open)} onNavigate={setLocation} onLogout={handleLogout} />
...
function CompanySignals({ profile }: { profile: ReturnType<typeof getCompanyHierarchy> }) {
  const socialLinks = [
    ["LinkedIn", profile.parentLinkedinProfile, Linkedin],
    ["Facebook", profile.parentFacebookPage, Facebook],
    ["Instagram", profile.parentInstagram, Instagram],
    ["X", profile.parentXProfile, MessageCircle],
  ] as const;
  return <div className="mx-5 mb-5 rounded-lg border border-violet-500/20 bg-violet-500/5 p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xs font-semibold text-white">Company intelligence signals</h3><p className="mt-1 text-[10px] text-slate-500">Use public social profiles and recent signals to personalize the parent-company conversation.</p></div><Activity className="h-4 w-4 text-violet-300" /></div><div className="mt-3 flex flex-wrap gap-2">{socialLinks.filter(([, value]) => Boolean(value)).map(([label, value, Icon]) => <a key={label} href={externalUrl(value!)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-md border border-slate-700 bg-slate-900/60 px-2.5 py-1.5 text-[10px] text-slate-200 hover:border-violet-400 hover:text-white"><Icon className="h-3.5 w-3.5 text-violet-300" />{label}<ExternalLink className="h-3 w-3 text-slate-500" /></a>)}</div>{profile.latestNews && <div className="mt-4 rounded-md border border-slate-800 bg-[#06101c] p-3"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-slate-500"><FileText className="h-3.5 w-3.5" />Latest news / intelligence signal <span className="text-slate-600">· {profile.latestNews.publishedAt}</span></div><div className="mt-2 text-xs text-slate-200">{profile.latestNews.headline}</div><div className="mt-1 text-[10px] text-slate-500">Source: {profile.latestNews.source}{profile.latestNews.demoOnly ? " · Demo data" : ""}</div>{profile.latestNews.url && <a href={externalUrl(profile.latestNews.url)} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[10px] text-violet-300 hover:text-white">Open source <ExternalLink className="h-3 w-3" /></a>}</div>}</div>;
}
