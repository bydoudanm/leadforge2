      ...
      <div className="flex min-h-[calc(100vh-4rem)]">
        ...
        <main className="flex-1 min-w-0 p-3 lg:p-5 space-y-3">
          ...
          <section className="rounded-xl border border-slate-800 bg-[#071321]/90 p-4 lg:p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-2.5">
              {opportunityOptions.map(([title, subtitle, iconName]) => {
                ...
                const tooltip = title === "Weak Social Media" ? "Targets businesses with limited activity or a weak presence across social channels." : undefined;
                const card = <button key={title} title={tooltip} onClick={() => toggle(selectedOpportunities, title, setSelectedOpportunities)} className={`min-h-[56px] rounded-lg border px-3 py-2 text-left flex items-center gap-2.5 transition-colors ${active ? "border-violet-500 bg-violet-500/10" : "border-slate-800 bg-slate-900/25 hover:border-slate-700"}`}><Icon className={`w-4 h-4 shrink-0 ${active ? "text-violet-400" : "text-slate-500"}`} /><span className="min-w-0"><span className="block text-[11px] font-medium text-slate-200 truncate">{title}</span><span className="block text-[10px] text-slate-500 truncate">{subtitle}</span></span>{active && <Check className="w-3.5 h-3.5 ml-auto text-violet-300 shrink-0" />}</button>;
                return tooltip ? <Tooltip key={title}><TooltipTrigger asChild>{card}</TooltipTrigger><TooltipContent side="top">{tooltip}</TooltipContent></Tooltip> : card;
              })}
            </div>
          </section>
          ...
          <section className="rounded-xl border border-slate-800 bg-[#071321]/90 p-4 lg:p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-2.5">
              {contactOptions.map(([label, Icon, pro]) => { const active = selectedContacts.includes(label); const tooltip = label === "Facebook Page" ? "Finds a public Facebook Page connected to the business." : label === "Instagram" ? "Finds a public Instagram profile connected to the business." : label === "LinkedIn Profile" ? "Finds a company or organization LinkedIn profile." : undefined; const card = <button key={label} title={tooltip} onClick={() => toggle(selectedContacts, label, setSelectedContacts)} className={`min-h-[48px] rounded-lg border px-3 py-2 text-left flex items-center gap-2 ${active ? "border-violet-500 bg-violet-500/10" : "border-slate-800 bg-slate-900/25 hover:border-slate-700"}`}><Icon className={`w-4 h-4 shrink-0 ${active ? "text-violet-300" : "text-slate-500"}`} /><span className="text-[10px] text-slate-200 truncate">{label}</span>{pro && <span className="ml-auto text-[8px] px-1.5 py-0.5 rounded bg-red-900/80 text-red-200">PRO</span>}{active && <Check className="w-3.5 h-3.5 text-violet-300 shrink-0" />}</button>; return tooltip ? <Tooltip key={label}><TooltipTrigger asChild>{card}</TooltipTrigger><TooltipContent side="top">{tooltip}</TooltipContent></Tooltip> : card; })}
            </div>
