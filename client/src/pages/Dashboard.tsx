    ...
    <div className="flex min-h-screen bg-slate-950">
      <aside className={`${sidebarOpen ? "w-64" : "w-20"} shrink-0 bg-slate-900 border-r border-slate-800 transition-all duration-300 flex flex-col`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {[{ icon: "📊", label: "Dashboard" }, { icon: "👥", label: "Leads" }, { icon: "📧", label: "Campaigns" }, { icon: "📈", label: "Analytics" }, { icon: "🎯", label: "Opportunities" }].map((item) => (
            <div key={item.label} className={`flex items-center gap-3 px-4 py-3 rounded-lg ${item.label === "Dashboard" ? "bg-blue-600/20 text-blue-400 border border-blue-500/30" : "text-slate-400"}`}>
